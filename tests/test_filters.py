...
@pytest.mark.django_db
def test_related_startswith_lookup(order_data):
    rows = Report(queryset=order_data, filters={"product__category__startswith": "B"}).run()
    assert len(rows) == 2
...
def test_invalid_related_lookup(order_data):
    with pytest.raises(InvalidFilterError, match="Unsupported lookup 'notalookup'"):
        Report(queryset=order_data, filters={"product__category__notalookup": "B"}).run()
